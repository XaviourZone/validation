...
def install_forwarder_web_extension(handler_class, workspace_root, service_controller, service_name, logger):
    """Install Forwarder API routes and dashboard injection without changing Router/Parser handlers."""
    ...
    def do_get(self):
        path = __import__("urllib.parse", fromlist=["urlparse"]).urlparse(self.path).path
    ...
    def serve_dashboard(self):
        """Serve the existing console plus the local Forwarder UI extension."""
        ...
        try:
            ...
            self._error_response(500, f"Error reading dashboard: {exc}")
    ...
    def _write_forwarder_files(self, config, secrets):
        ...
        try:
            ...
            if os.name != "nt":
                os.chmod(self.forwarder_secret_path, 0o600)
        finally:
            try:
                os.unlink(tmp_name)
            except FileNotFoundError:
    ...
    def _forwarder_status(self):
        ...
        if reachable:
            status = "RUNNING"
        return self._json_response({"service": "validation-data-forwarder", "status": status, "reachable": reachable, "service_info": svc, "health": health})

    def _forwarder_proxy_get(self, endpoint):
        try:
            self._json_response(_json_request(f"{_API_URL}{endpoint}"))
        except Exception as exc:
            self._json_response({"reachable": False, "error": str(exc)}, status=503)
    ...
    def _forwarder_post(self, path):
        ...
        if path == "/api/forwarder/reload":
            self._forwarder_service_api("/config/reload", {})
            return
        if path == "/api/forwarder/test":
            self._forwarder_service_api("/config/destinations/test", {"name": body.get("name", "")})
            return
        ...
        if match:
            ...
            return
        self._error_response(404, "Forwarder endpoint not found")

    def _save_destination(self, body):
        name = str(body.get("name", "")).strip()
        if not name or not re.fullmatch(r"[A-Za-z0-9_-]+", name):
            self._error_response(422, "Destination name must contain only letters, numbers, '_' or '-'")
            return
        protocol = str(body.get("protocol", "sftp")).lower()
        if protocol not in ("sftp", "filesystem"):
            self._error_response(422, "Unsupported protocol")
            return
        try:
            port = int(body.get("port", 22 if protocol == "sftp" else 1))
            timeout = int(body.get("connect_timeout_seconds", 10))
        except (TypeError, ValueError):
            self._error_response(422, "Port and timeout must be numeric")
            return
        if port < 1 or port > 65535 or timeout < 1:
            self._error_response(422, "Invalid port or timeout")
            return
        if protocol == "sftp" and not all(str(body.get(k, "")).strip() for k in ("host", "remote_path", "username")):
            self._error_response(422, "SFTP requires host, remote path and username")
            return

        cfg = self._forwarder_config()
        cfg.setdefault("destinations", {})[name] = {
            ...
            "verify_remote_size": bool(body.get("verify_remote_size", True)),
        }
        secrets = self._forwarder_secrets()
        password = body.get("password")
        if password:
            secrets[name] = str(password)
    ...
    def _modify_destination(self, name, action):
        ...
        if name not in destinations:
            self._error_response(404, f"Destination '{name}' not found")
            return
        if action == "delete":
    ...
    def _forwarder_service_api(self, endpoint, payload):
        try:
            result = _json_request(f"{_API_URL}{endpoint}", method="POST", payload=payload, timeout=5)
            self._json_response(result)
        except Exception as exc:
            # Configuration is already persisted locally. A stopped service can
            # be started/restarted by the operator to load it.
            self._json_response({"success": True, "reload_required": True, "message": f"Configuration saved; Forwarder reload pending ({exc})"})
