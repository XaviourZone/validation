...
(function () {
      ...
      <div class="modal-backdrop" id="fw-modal">
        <div class="modal-container" style="max-width:700px;">
          ...
          <div class="modal-body">
            ...
            <div class="form-row">
              <div class="form-group"><label class="form-label">Host / IP *</label><input id="fw-host" class="form-input" placeholder="40.1.1.1"></div>
              <div class="form-group"><label class="form-label">SSH Port</label><input id="fw-port" type="number" class="form-input" value="22" min="1" max="65535"></div>
            </div>
            <div class="form-group"><label class="form-label">SSH Username *</label><input id="fw-user" class="form-input" placeholder="ddiode"></div>
            <div class="form-group"><label class="form-label">SSH Password</label><input id="fw-password" type="password" class="form-input" autocomplete="new-password" placeholder="Enter only when needed"><div id="fw-password-hint" class="form-hint"></div></div>
            ...
            <div class="form-row">
              <div class="form-group"><label class="form-label">Connection Timeout</label><input id="fw-timeout" type="number" class="form-input" value="10" min="1"></div>
              <div class="form-group" style="display:flex;align-items:center;padding-top:24px;"><label style="display:flex;align-items:center;gap:8px;font-size:13px;"><input type="checkbox" id="fw-enabled"> Enable</label></div>
            </div>
  ...
  function setStatus(status) {
    ...
    el.textContent=s; el.className=`status-pill ${s==="RUNNING"?"running":(s==="STARTING"?"degraded":"stopped")}`;
  }
  ...
  async function refreshDestinations(){
    try{
      ...
      (d.destinations||[]).forEach(dest=>{
        ...
        tr.innerHTML=`<td><strong>${esc(dest.name)}</strong></td><td><span class="status-pill ${dest.enabled?"running":"stopped"}" style="font-size:9px;">${dest.enabled?"ENABLED":"DISABLED"}</span></td><td class="mono">SSH ${esc(dest.host)}:${esc(dest.port)}</td><td class="mono">${esc(dest.remote_path)}</td><td>${esc(dest.username)}</td><td>${dest.password_configured?"Configured":"Not configured"}</td><td><div class="btn-group"><button class="btn btn-secondary fw-edit" data-name="${esc(dest.name)}">Edit</button><button class="btn btn-secondary fw-test-row" data-name="${esc(dest.name)}">Test</button><button class="btn ${dest.enabled?"btn-danger":"btn-success"} fw-toggle" data-name="${esc(dest.name)}">${dest.enabled?"Disable":"Enable"}</button><button class="btn btn-danger fw-delete" data-name="${esc(dest.name)}">Delete</button></div></td>`;
        body.appendChild(tr);
      });
  ...
  function openModal(dest){
    const modal=document.getElementById("fw-modal"); modal.classList.add("open");
    ...
    hint.textContent=dest?(dest.password_configured?"Password configured. Leave blank to keep it.":"Password NOT configured. Enter the Data Diode SSH password before saving."):"Enter the Data Diode SSH password.";
    const error=document.getElementById("fw-form-error");error.style.display="none";error.textContent="";
  }
  ...
  async function saveDestination(){
    ...
    try{const d=await api("/api/forwarder/destinations/save",{method:"POST",body:JSON.stringify(p)});if(d.password_required)throw new Error(d.error);closeModal();refresh();}catch(e){err.textContent=e.message;err.style.display="block";err.style.color="var(--accent-rose)";}
  }

  async function testCurrent(){
    const p=getDestinationPayload(),err=document.getElementById("fw-form-error");
    if(!p.name||!p.host||!p.remote_path||!p.username){err.textContent="Name, host, remote folder and SSH username are required.";err.style.display="block";return;}
    try{await api("/api/forwarder/destinations/save",{method:"POST",body:JSON.stringify(p)});const d=await api("/api/forwarder/test",{method:"POST",body:JSON.stringify({name:p.name})});err.textContent=d.message||"SSH connection test succeeded.";err.style.display="block";err.style.color="var(--accent-emerald)";refresh();}catch(e){err.textContent="SSH test failed: "+e.message;err.style.display="block";err.style.color="var(--accent-rose)";}
  }
