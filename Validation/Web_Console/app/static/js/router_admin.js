/* Router administration extension: filesystem folder selection, file-pattern presets,
   parser mapping editor, parser creation and automatic Router reload after source changes. */
(function () {
  "use strict";
  ...
  function openFolderBrowser(target) {
    ...
    if (!modal) {
      ...
      modal.innerHTML = `<div class="modal-container" style="max-width:780px;">
        ...
        <div class="modal-body"><div class="form-group"><label class="form-label">Current folder</label><input id="rfb-path" class="form-input" value="/"></div>
      ...
      document.getElementById("rfb-up").onclick = async () => { const d = await browse(document.getElementById("rfb-path").value); if (d.parent) browse(d.parent); };
    }
  ...
  async function createParser() {
    const name = prompt("New parser name (example: CUSTOM_RADAR):"); if (!name) return;
    const portText = prompt("TCP parser port (1-65535):", "10006"); if (!portText) return;
    const port = Number(portText); if (!Number.isInteger(port) || port < 1 || port > 65535) { alert("Invalid parser port."); return; }
    try {
      await api("/api/parser/mapping/create", {method:"POST", body:JSON.stringify({parser:name.trim(), port})});
      alert(`Parser ${name.trim()} created. Its 41-field mapping is ready to configure.`);
      openMapping();
  ...
  async function openMapping() {
    ...
    if (!modal) {
      ...
      modal.innerHTML=`<div class="modal-container" style="max-width:1200px;width:96vw;max-height:92vh;">
        ...
        </div><div style="font-size:11px;color:var(--text-muted);margin:8px 0 12px;">Priority is left-to-right. Sources: incoming:&lt;field&gt;, ais_state:&lt;field&gt;, wrs:&lt;field&gt;, pans:&lt;field&gt;, nsc:&lt;field&gt;, default:&lt;value&gt;.</div>
  ...
  function boot(){installFetchReload();ensureBrowseButton();installMappingPanel();setTimeout(()=>{ensureBrowseButton();installMappingPanel();},1000);}
