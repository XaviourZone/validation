"""Position-based AIS anomaly detection.

Compares consecutive valid AIS positions for one MMSI using source timestamps.
This module only evaluates kinematic plausibility; it does not decide vessel
identity and does not replace WRS/PANS/NSC correlation.
"""
...
class PositionalSpoofingDetector:
    """Calculate consecutive-position speed and flag physically implausible motion."""
    ...
    def _load_config(self) -> Dict[str, Any]:
        if not self.config_path.exists():
            return {"enabled": True, "max_speed_knots": {"default": self.DEFAULT_MAX_SPEED_KNOTS}}
        try:
    ...
    def _threshold(self, vessel_type: Any) -> float:
        ...
        for key in ("high_speed", "passenger", "tanker", "cargo", "commercial", "fishing"):
            if key in text and limits.get(key) is not None:
                return float(limits[key])
        try:
            return float(limits.get("default", self.DEFAULT_MAX_SPEED_KNOTS))
        except (TypeError, ValueError):
            return self.DEFAULT_MAX_SPEED_KNOTS

    def check(self, previous: Optional[Dict[str, Any]], current: Any) -> Optional[Dict[str, Any]]:
        if not self.config.get("enabled", True) or not previous:
            return None

        cur_lat = _number(getattr(current, "latitude", None))
        cur_lon = _number(getattr(current, "longitude", None))
        ...
        if None in (cur_lat, cur_lon, cur_ts, prev_lat, prev_lon, prev_ts):
            return None
        if not (-90 <= cur_lat <= 90 and -180 <= cur_lon <= 180 and -90 <= prev_lat <= 90 and -180 <= prev_lon <= 180):
            return None
        ...
        if speed_knots > threshold:
            return {
                "flagged": True,
                "reason": "POSITIONAL SPOOFING",
                "distance_nm": round(distance_nm, 3),
                "elapsed_seconds": round(elapsed, 3),
                "calculated_speed_knots": round(speed_knots, 3),
                "reported_sog_knots": round(reported_sog, 3) if reported_sog is not None else None,
                "threshold_knots": threshold,
                "previous_timestamp": cur_ts.fromtimestamp(cur_ts.timestamp(), tz=timezone.utc).isoformat(),
            }
        return None
