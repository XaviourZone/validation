# Validation offline deployment bundle preparation
# Run this script on an INTERNET-CONNECTED Windows machine.
# It downloads the Linux x86_64 Python runtime and all Python wheels needed
# by the current Validation services.
...
if (-not (Test-Path $PythonArchive)) {
    ...
    Write-Host "[SKIP] Python archive already exists."
}
...
if ($actualHash -ne $PythonSha256) {
    throw "Python runtime SHA256 mismatch. Expected $PythonSha256 but got $actualHash"
}
